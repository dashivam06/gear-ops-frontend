...
export default function SettingsPage() {
  ...
  const memberSince = user?.joinDate ? format(new Date(user.joinDate), "MMMM yyyy") : format(new Date(), "MMMM yyyy");
              ...
              <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
                ...
                <div className="min-w-0 flex-1 space-y-5">
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div className="rounded-xl border border-zinc-100 bg-zinc-50/80 p-4">
                      <p className="text-xs font-medium uppercase tracking-wide text-zinc-400">Position</p>
                      <p className="mt-1 text-sm font-medium text-zinc-900">{user?.position || "Not provided"}</p>
                    </div>
                    <div className="rounded-xl border border-zinc-100 bg-zinc-50/80 p-4">
                      <p className="text-xs font-medium uppercase tracking-wide text-zinc-400">Status</p>
                      <p className="mt-1 text-sm font-medium text-zinc-900">{user?.status || "Not provided"}</p>
                    </div>
                  </div>
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const updatedProfile = await updateStaffProfile(accessToken || "", {
                        fullName: fullName.trim() || user?.fullName || "",
                        phone: phone.trim(),
                        address: address.trim(),
                        profileImageUrl: payload.profileImage ?? user?.profileImageUrl ?? "",
                      });
                      const mappedProfile = {
                         ...(user || {}),
                         ...
                         joinDate: updatedProfile.joinDate ?? user?.joinDate,
                         status: updatedProfile.status ?? user?.status,
                      };
                      setUser(mappedProfile);
                      setAuthProfileStorage(mappedProfile);
                      setFullName(updatedProfile.fullName);
                      setPhone(updatedProfile.phone ?? phone.trim());
                      setAddress(updatedProfile.address ?? address.trim());
